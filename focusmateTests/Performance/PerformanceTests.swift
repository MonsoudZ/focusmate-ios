...
final class PerformanceTests: XCTestCase {
  ...
  func testGroupedTasksPerformance_50Tasks() {
    let tasks = self.generateTasks(count: 50)
    let todayData = self.makeTodayResponse(dueTodayTasks: tasks)

    measure {
      _ = self.groupTasks(from: todayData)
    }
  }

  /// Tests groupedTasks computation with 100 tasks
  func testGroupedTasksPerformance_100Tasks() {
    let tasks = self.generateTasks(count: 100)
    let todayData = self.makeTodayResponse(dueTodayTasks: tasks)
  ...
  func testGroupedTasksPerformance_500Tasks() {
    let tasks = self.generateTasks(count: 500)
    let todayData = self.makeTodayResponse(dueTodayTasks: tasks)
  ...
  func testGroupedTasksPerformance_1000Tasks() {
    let tasks = self.generateTasks(count: 1000)
    let todayData = self.makeTodayResponse(dueTodayTasks: tasks)
  ...
  func testCacheWritePerformance_100Entries() async {
  ...
  func testCacheReadPerformance_100Entries() async {
    let cache = ResponseCache.shared
    await cache.invalidateAll()
  ...
  func testCachePerformance_DTOArrays() async {
    let cache = ResponseCache.shared
    await cache.invalidateAll()
  ...
  func testDateParsingPerformance() {
  ...
  func testTaskDueDateParsingPerformance() {
    let tasks = self.generateTasks(count: 100)
  ...
  func testOverdueFilterPerformance_500Tasks() {
    let tasks = self.generateTasks(count: 500, overduePercentage: 0.2)

    measure {
      let overdue = tasks.filter(\.isActuallyOverdue)
      _ = overdue.count
    }
  }

  /// Tests filtering completed tasks
  func testCompletedFilterPerformance_500Tasks() {
    let tasks = self.generateTasks(count: 500, completedPercentage: 0.5)
  ...
  func testTaskSortingByDueDate_500Tasks() {
    let tasks = self.generateTasks(count: 500)
  ...
  func testTaskSortingByPriorityAndDueDate_500Tasks() {
    let tasks = self.generateTasks(count: 500)
  ...
  func testMemoryFootprint_1000Tasks() {
  ...
  private func groupTasks(from data: TodayResponse)
    -> (anytime: [TaskDTO], morning: [TaskDTO], afternoon: [TaskDTO], evening: [TaskDTO])
  {
