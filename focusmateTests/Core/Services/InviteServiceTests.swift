...
final class InviteServiceTests: XCTestCase {
    ...
    override func setUp() {
        ...
        service = InviteService(apiClient: apiClient)
    }
    ...
    func testFetchInvitesDecodesResponse() async throws {
        ...
        mock.stubJSON(InvitesResponse(invites: [invite]))
    ...
    func testFetchInvitesCallsCorrectEndpoint() async throws {
        mock.stubJSON(InvitesResponse(invites: []))

        _ = try await service.fetchInvites(listId: 42)

        XCTAssertEqual(mock.lastCall?.method, "GET")
        XCTAssertEqual(mock.lastCall?.path, API.Lists.invites("42"))
    }

    func testFetchInvitesRejectsNegativeListId() async {
        do {
            _ = try await service.fetchInvites(listId: -1)
    ...
    func testCreateInviteSendsCorrectBody() async throws {
        ...
        mock.stubJSON(InviteResponse(invite: invite))

        _ = try await service.createInvite(listId: 5, role: "editor", maxUses: 10)

        XCTAssertEqual(mock.lastCall?.method, "POST")
        XCTAssertEqual(mock.lastCall?.path, API.Lists.invites("5"))
    ...
    func testCreateInviteCallsCorrectEndpoint() async throws {
        mock.stubJSON(InviteResponse(invite: TestFactories.makeSampleInvite()))

        _ = try await service.createInvite(listId: 3)

        XCTAssertEqual(mock.lastCall?.method, "POST")
        XCTAssertEqual(mock.lastCall?.path, API.Lists.invites("3"))
    }

    func testCreateInviteOmitsNilOptionals() async throws {
        mock.stubJSON(InviteResponse(invite: TestFactories.makeSampleInvite()))

        _ = try await service.createInvite(listId: 1)
    ...
    func testRevokeInviteCallsCorrectEndpoint() async throws {
        try await service.revokeInvite(listId: 5, inviteId: 10)

        XCTAssertEqual(mock.lastCall?.method, "DELETE")
        XCTAssertEqual(mock.lastCall?.path, API.Lists.invite("5", "10"))
    }

    func testRevokeInviteRejectsNegativeListId() async {
        do {
            try await service.revokeInvite(listId: -1, inviteId: 1)
            XCTFail("Expected validation error")
        } catch {
            // Validation rejects negative IDs
        }
    ...
    func testPreviewInviteDecodesResponse() async throws {
        ...
        mock.stubJSON(InvitePreviewResponse(invite: preview))
    ...
    func testPreviewInviteCallsCorrectEndpoint() async throws {
        mock.stubJSON(InvitePreviewResponse(invite: TestFactories.makeSampleInvitePreview()))

        _ = try await service.previewInvite(code: "ABC")

        XCTAssertEqual(mock.lastCall?.method, "GET")
        XCTAssertEqual(mock.lastCall?.path, API.Invites.preview("ABC"))
    }

    func testPreviewInviteRejectsEmptyCode() async {
        do {
            _ = try await service.previewInvite(code: "")
            XCTFail("Expected validation error")
        } catch {
    ...
    func testAcceptInviteDecodesResponse() async throws {
        ...
        mock.stubJSON(AcceptInviteResponse(list: list, membership: membership))
    ...
    func testAcceptInviteCallsCorrectEndpoint() async throws {
        ...
        _ = try await service.acceptInvite(code: "XYZ")

        XCTAssertEqual(mock.lastCall?.method, "POST")
        XCTAssertEqual(mock.lastCall?.path, API.Invites.accept("XYZ"))
    }

    func testAcceptInviteRejectsEmptyCode() async {
        do {
            _ = try await service.acceptInvite(code: "")
            XCTFail("Expected validation error")
        } catch {
            // Validation rejects empty strings
        }
