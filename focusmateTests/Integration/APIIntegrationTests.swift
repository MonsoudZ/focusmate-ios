...
final class APIIntegrationTests: XCTestCase {
  ...
  override func setUp() async throws {
    ...
    self.apiClient = APIClient(tokenProvider: { "test-token" }, networking: self.mockNetworking)
  ...
  func testCreateTaskFlow() async throws {
    ...
    XCTAssertEqual(self.mockNetworking.lastCall?.method, "POST")
    XCTAssertTrue(self.mockNetworking.lastCall?.path.contains("/tasks") ?? false)
  }
  ...
  func testUpdateTaskFlow() async throws {
    ...
    self.mockNetworking.stubJSON(SingleTaskResponse(task: updatedTask))
    ...
    XCTAssertEqual(self.mockNetworking.lastCall?.method, "PUT")
  }

  func testDeleteTaskFlow() async throws {
    // Given: An existing task
    self.mockNetworking.stubbedData = Data()

    // When: Deleting the task
    try await self.taskService.deleteTask(listId: 1, taskId: 1)

    // Then: DELETE request is sent
    XCTAssertEqual(self.mockNetworking.lastCall?.method, "DELETE")
    XCTAssertTrue(self.mockNetworking.lastCall?.path.contains("/tasks/1") ?? false)
  }
  ...
  func testCompleteTaskFlow() async throws {
    ...
    self.mockNetworking.stubJSON(SingleTaskResponse(task: completedTask))
    ...
    XCTAssertEqual(self.mockNetworking.lastCall?.method, "PATCH")
    XCTAssertTrue(self.mockNetworking.lastCall?.path.contains("/complete") ?? false)
  }
  ...
  func testFetchListsFlow() async throws {
    ...
    self.mockNetworking.stubJSON(ListsResponse(lists: lists, tombstones: nil))
  ...
  func testCreateListFlow() async throws {
    ...
    self.mockNetworking.stubJSON(ListResponse(list: createdList))
  ...
  func testDeleteListFlow() async throws {
    ...
    try await self.listService.deleteList(id: 1)
  ...
  func testNetworkErrorHandling() async throws {
    ...
    self.mockNetworking.stubbedError = APIError.noInternetConnection

    // When: Making a request
    do {
      _ = try await self.taskService.fetchTasks(listId: 1)
  ...
  func testUnauthorizedErrorHandling() async throws {
    ...
    self.mockNetworking.stubbedError = APIError.unauthorized("Session expired")
  ...
  func testServerErrorHandling() async throws {
    ...
    self.mockNetworking.stubbedError = APIError.serverError(500, "Internal Server Error", nil)
  ...
  func testCreateSubtaskRequestIsSent() async throws {
    ...
    self.mockNetworking.stubJSON(SingleTaskResponse(task: TestFactories.makeSampleTask(id: 100, title: "Subtask")))

    // When: Creating a subtask
    do {
      _ = try await self.subtaskManager.create(parentTask: parentTask, title: "Subtask")
    } catch {
  ...
  func testFetchTagsFlow() async throws {
    ...
    self.mockNetworking.stubJSON(TagsResponse(tags: tags))
  ...
  func testRequestIncludesCorrectHeaders() async throws {
    ...
    self.mockNetworking.stubJSON(TasksResponse(tasks: [task], tombstones: nil))

    // When: Making a request
    _ = try await self.taskService.fetchTasks(listId: 1)

    // Then: Request is made to correct endpoint
    XCTAssertNotNil(self.mockNetworking.lastCall)
    XCTAssertEqual(self.mockNetworking.lastCall?.method, "GET")
  }
  ...
  func testCreateTaskRequestIsSentCorrectly() async throws {
    ...
    XCTAssertEqual(self.mockNetworking.lastCall?.method, "POST")
    XCTAssertTrue(self.mockNetworking.lastCall?.path.contains("/tasks") ?? false)
    XCTAssertNotNil(self.mockNetworking.lastCall?.body)
  }
  ...
  func testMultipleSequentialRequests() async throws {
    ...
    XCTAssertEqual(self.mockNetworking.calls.count, 2)
  }
}
