...
final class TodayViewModelTests: XCTestCase {
  ...
  func testToggleCompleteReopensCompletedTask() async {
    ...
    self.stubTodayResponse(dueToday: [TestFactories.makeSampleTask(id: 1, title: "Reopened Task")])
  ...
  func testGroupedTasksWithNoDueDate() async {
    ...
    self.stubTodayResponse(dueToday: [taskNoDue])
  ...
  func testGuardDropsFutureTaskFromOverdue() async {
    ...
    self.stubTodayResponse(overdue: [futureTask])
  ...
  func testGuardKeepsYesterdayTaskInServerBucket() async {
    ...
    self.stubTodayResponse(dueToday: [yesterdayTask])

    let vm = self.makeViewModel()
    await vm.loadToday()

    XCTAssertEqual(
      vm.todayData?.due_today.count,
      1,
      "Past tasks stay in server's bucket — guard only strips future leaks"
    )
  }
  ...
  func testGuardTrustsCompletedTodayFromServer() async {
    ...
    self.stubTodayResponse(completedToday: [completedTask])
  ...
  func testGuardSkipsRebuildWhenNothingRemoved() async {
    ...
    self.stubTodayResponse(dueToday: [task1, task2])
  ...
  func testGuardKeepsNoDueDateTaskInServerBucket() async {
    ...
    self.stubTodayResponse(dueToday: [noDueDateTask])
