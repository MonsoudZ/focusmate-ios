...
enum ErrorMapper {
  ...
  static func mapAPIError(_ error: APIError) -> FocusmateError {
    ...
    case let .badStatus(code, message, details):
      return self.mapHTTPStatus(code: code, message: message, details: details)
    case let .rateLimited(seconds):
      return .rateLimited(seconds, nil)
    case let .serverError(code, message, details):
      return .serverError(code, message, self.extractMessage(from: details))
  ...
  static func mapHTTPStatus(code: Int, message: String?, details: [String: Any]?) -> FocusmateError {
    switch code {
    case 400:
      return .badRequest(message ?? "Bad request", self.extractMessage(from: details))
    case 401:
    ...
    case 422:
      ...
      return .badRequest(message ?? "Validation failed", self.extractMessage(from: details))
    case 429:
