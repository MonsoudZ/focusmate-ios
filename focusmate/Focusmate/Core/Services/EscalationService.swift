...
final class EscalationService {
  ...
  func getBlockingTasks() async throws -> [BlockingTask] {
    let response: BlockingTasksResponse = try await apiClient.request(
      "GET",
  ...
  func addExplanation(itemId: Int, explanation: String) async throws -> TaskExplanation {
    ...
    let response: ExplanationResponse = try await apiClient.request(
      "POST",
      "items/\(itemId)/explanations",
      body: request
    )
  ...
  func getExplanations(itemId: Int) async throws -> [TaskExplanation] {
    let response: ExplanationsResponse = try await apiClient.request(
      "GET",
      "items/\(itemId)/explanations",
      body: nil as String?
    )
  ...
  func resolveEscalation(escalationId: Int, resolution: String) async throws -> Escalation {
    ...
    let response: EscalationResponse = try await apiClient.request(
      "PATCH",
