...
final class CalendarService {
  ...
  private var eventsCacheTTL: TimeInterval {
    AppConfiguration.Cache.calendarEventsTTLSeconds
  }
  ...
  private func getCachedCalendar() -> EKCalendar? {
    ...
    self.cachedCalendar = calendars.first(where: { $0.title == self.calendarName })
    return self.cachedCalendar
  }

  private func getCachedEvents(for calendar: EKCalendar) -> [EKEvent] {
    if Date().timeIntervalSince(self.eventsCacheTimestamp) < self.eventsCacheTTL {
    ...
    self.cachedEvents = self.eventStore.events(matching: predicate)
    self.eventsCacheTimestamp = Date()
    return self.cachedEvents
  }

  private func invalidateEventsCache() {
    self.eventsCacheTimestamp = .distantPast
  }
  ...
  func addTaskToCalendar(_ task: TaskDTO) {
    ...
    self.removeTaskFromCalendar(taskId: task.id)
  ...
  func removeTaskFromCalendar(taskId: Int) {
    ...
    let events = self.getCachedEvents(for: calendar)
