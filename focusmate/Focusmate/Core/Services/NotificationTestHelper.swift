...
import UserNotifications
  ...
  final class NotificationTestHelper {
    ...
    func refreshScheduledNotifications() async {
      ...
      self.scheduledNotifications = requests.map { request in
        let triggerDate: Date?
        let triggerType: String
    ...
    func scheduleEscalationStartTest(
      taskId: Int = 999,
      taskTitle: String = "Test Task",
    ...
    func scheduleEscalationWarningTest(
      taskId: Int = 999,
      taskTitle: String = "Test Task",
      delaySeconds: TimeInterval = 5
    ) {
    ...
    func simulateTaskNotificationTap(taskId: Int) {
      ...
      if let route {
        self.lastSimulatedRoute = "Task \(taskId)"
    ...
    func simulateNudgePush(taskId: Int) {
      ...
      if let route {
        self.lastSimulatedRoute = "Nudge → Task \(taskId)"
        AppRouter.shared.handleDeepLink(route)
    ...
    func clearAllNotifications() {
      ...
      Task {
        await self.refreshScheduledNotifications()
      }
