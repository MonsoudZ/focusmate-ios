...
final class AdvancedErrorHandler: ObservableObject {
  ...
  func shouldRetry(error: FocusmateError, context: String) -> Bool {
    self.retryCoordinator.shouldRetry(error: error, context: context)
  }
  ...
  func retryWithBackoff<T>(
    ...
    try await self.retryCoordinator.retryWithBackoff(context: context, error: error, operation: operation)
  }
